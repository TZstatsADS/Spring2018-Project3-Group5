...
```{r wkdir, eval=FALSE}
setwd("D:/Personal/Documents/GitHub/Spring2018-Project3-Group5")
```
...
img_train_dir <- paste(experiment_dir, "training_set/images/", sep="")
img_test_dir <- paste(experiment_dir, "testing_set/images/", sep="")
```
...
run.feature.test=F # process features for testing pictures
...
```{r train_label}
labels<-read.csv("../data/training_set/label_train.csv")
...
# get SIFT feature
  sift_feature_train <- read.csv("../data/training_set/SIFT_train.csv",header = F)
  sift_feature_train <- sift_feature_train[,-1]
  sift_feature_train$y <- label_train
  saveRDS(sift_feature_train, file="../output/feature_SIFT_train.RData")
...
#load SIFT feature
  sift_feature_train<-readRDS("../output/feature_SIFT_train.RData")
#load RGB feature
  rgb_feature_train<-readRDS('../output/feature_rgb_train.RData')
...
# get SIFT feature
  sift_feature_test <- read.csv("../data/testing_set/SIFT_train.csv",header = F)
  sift_feature_test <- sift_feature_test[,-1]
  saveRDS(sift_feature_test, file="../output/feature_SIFT_test.RData")

# get RGB feature
  tm_feature <- system.time(rgb_feature_test <- feature(img_train_dir, 
        "test", 
        data_name="rgb", 
        export=TRUE))[3]
}
...
# train model
  ...
  saveRDS(base_model,file='../output/base_model.RData')
}else{
  base_model <- readRDS('../output/base_model.RData')
}
```
...
# train model
  ...
  saveRDS(best_model,file='../output/best_model.RData')
  best_model <- readRDS('../output/best_model.RData')}
```

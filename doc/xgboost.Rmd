...
training_x_hog<- feature[ ,!(colnames(feature) %in% c("y"))]



xgb_cv.function(trainning_x_hog, trainig_y_hog, 5)
para_hog<-xgb_para(training_x_hog, training_y_hog, 5)
...
xgb_cv.function(training_x_RGB, training_y_RGB, 5)
para_RGB<-xgb_para(training_x_RGB, training_y_RGB, 5)
...
xgb_cv.function(training_x_SIFT, training_y_SIFT, 5)
para_lbp<-xgb_para(training_x_SIFT, training_y_SIFT, 5)

```

```{r}
fit_hog_xgboost<-xgb_train(training_x_hog, as.factor(label), para_hog[[2]])
p<-round(predict(fit_hog_xgboost, as.matrix(training_x_hog)))
p<-xgb_test(fit_hog_xgboost, training_x_hog)

fit2<-xgb_train(training_x_lbp, as.factor(label), para_lbp[[2]])
xgb_test(fit2, training_x_lbp)
p <- round(predict(fit2, as.matrix(training_x_lbp)))

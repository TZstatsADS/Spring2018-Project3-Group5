...
xgboost_para <- function(dat_train,label_train,K){
  ...
  for (i in 1:3){
    for (j in 1:3){
      ...
      set.seed(11)
      cv.output <- xgboost.cv(data=dtrain, params=my.params, 
...
fit_hog_xgboost<-xgboost_train(training_x_hog[train,], as.factor(label)[train], para_hog[[2]])
...
fit_rgb_xgboost<-xgboost_train(training_x_RGB[train,], as.factor(label)[train], para_RGB[[2]])
p_RGB<-round(predict(fit_rgb_xgboost, as.matrix(training_x_RGB[-train,])))
sum(p_RGB!=label[-train])/1000

fit_sift_xgboost<-xgboost_train(training_x_SIFT[train,], as.factor(label)[train], para_sift[[2]])
